 ...
 * light edges. Root must be 0.
 * $O((\log N)^2)$
 * Status: Tested on https://codeforces.com/contest/1904/problem/F
 */
...
struct HLD{
    ...
    HLD(vvi &adj) : n(sz(adj)), top(n), tin(n), p(n, -1), sub(n, 1) {
        ...
        for(int v=0; v < n; v++) 
            if(sz(adj[v])) iter_swap(begin(adj[v]), max_element(all(adj[v]), [&](int a, int b){
                return make_pair(a != p[v], sub[a]) < make_pair(b != p[v], sub[b]); }));
        function<void(int)> dfs = [&](int v){
            tin[v] = timer++;
            for(auto &to : adj[v]) if(to != p[v]) {
                top[to] = (to == adj[v][0] ? top[v] : to);
                dfs(to);
            }
        }; dfs(0);
    }
    int lca(int u, int v) { return process(u, v, [](...){}); }
